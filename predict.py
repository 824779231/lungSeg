...
def loadDCM(f, no_preprocess=False, dicom=False):
    ...
    h = (int((576 - hLoc) / 2)) + p
    w = int((448 - wLoc) / 2) + p
    roi = torch.zeros(pImg.shape, device=device)
    if w == p:
...
def saveMask(f, img, h, w, hLoc, wLoc, imH, imgW, no_post=True):
    img = img.data.numpy()
    imgIp = img.copy()
    ...
    if not no_post:
        ...
        imsave(f.replace('.jpg', '_post.jpg'), img_as_ubyte(imgPost > 0.5))


parser = argparse.ArgumentParser()
parser.add_argument('--data', type=str, default='testData/image/', help='Path to input files')
parser.add_argument('--model', type=str, default='saved_models/example.pt',
...
files = list(set(glob(args.data + '*.' + filetype)) - set(glob(args.data + '*_mask*.' + filetype)) - set(
    glob(args.data + '*label*.' + filetype)))
files = sorted(files)
for fIdx in range(len(files)):
    f = files[fIdx]
    fName = f.split('/')[-1]
    fRealname = os.path.split(f)[1]
    img, roi, h, w, hLoc, wLoc, imH, imW = loadDCM(f,no_preprocess=args.no_preprocess,dicom=args.dicom)
    img = img.to(device)
    mask,edg = net(img)
    ...
    f = save_dir + fRealname.replace('.' + filetype, '_mask.jpg')
    # print(f)
