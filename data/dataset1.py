...
def pad(data):
	...
	pImg = torch.zeros((1,512,512))
	h = (int((512-data.shape[1])/2) )
...
#		pdb.set_trace()
		if process:
			self.process()
		self.data, self.targets = torch.load('lungData6000.pt')
		self.p = p
		if self.rMask > 0:
			if not (self.block):
				self.masks = torch.load('blurry_masks_200.pt')
	...
	def process(self):
			...
			torch.save((images,labels),self.data_dir+'lungData6000.pt')
